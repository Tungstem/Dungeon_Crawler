...
x, y = Level_Creation_Random.fill_grid(level, level_row, level_column)

while playing == True:
    Level_Creation_Random.draw_grid(level, level_row, level_column)
    choice = On_Screen.show_menu()
    if choice == "1":
        x, y = General_Movement.hero_movement(x, y, level, level_row, level_column)  # I movimenti sono gestiti direttamente nel modulo on screen
    if choice == "2":
        On_Screen.item_menu()
    if choice == "3":
